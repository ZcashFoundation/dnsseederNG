    ...
    async fn handle_request<R: ResponseHandler>(
        ...
        if let Some(query) = request.queries().first() {
            ...
            if let Ok(book) = self.address_book.lock() {
                ...
                let mut matched_peers: Vec<&zebra_network::types::MetaAddr> = Self::filter_candidates(candidate_refs, record_type, default_port);
...
mod tests {
    ...
    fn test_filter_candidates_a_record() {
        ...
        let v6_good = mock_peer(IpAddr::V6(Ipv6Addr::new(0x2001, 0xdb8, 0, 0, 0, 0, 0, 1)), 8233);
